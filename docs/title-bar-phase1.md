...
4. The root renderer chrome (`frontend/src/components/layout/WindowControls.tsx`, mounted from `DesktopDragSurface` on every route) reads `titleBarMode` and the document's readiness `titleBarEpoch` from `getStatus()` and mounts minimal min/max/close buttons when main explicitly reports `'html-fallback'`; unrecognized or missing values keep native-overlay behavior so an older main never receives duplicate controls. Readiness is a coherent epoch-bound lifecycle owned by `main/window-readiness.ts`: every navigation (`did-start-loading`, initial load included) begins a new epoch; the renderer reports `windowReady({ epoch })` only once its control surface actually exists (native caption buttons are always present in overlay mode; the fallback report fires after the buttons commit to the DOM); reports bound to stale or malformed epochs are ignored, so a reload can never inherit a previous document's confirmation. A bounded fail-safe in main (10 seconds per epoch) shows the window and logs loudly if the current epoch never confirms - a visible window without verified controls beats an invisible POS forever. The buttons are pinned to the trailing edge, styled with logical CSS properties (RTL-safe), and invoke one narrow IPC verb: `windowAction('minimize' | 'toggle-maximize' | 'close')` exposed via `main/preload.ts`. Context isolation is unchanged.
...
6. Renderer readiness is per-document: main begins a fresh readiness epoch on window creation and on each `did-start-loading` navigation (manual reloads included), rejecting stale-epoch reports and arming a fresh fail-safe per epoch, so a reload that fails to re-mount the root chrome re-evaluates readiness instead of inheriting a stale ready flag while the caption controls are gone.

Contract coverage lives in `tests/titlebar-window-options.test.ts` (mode resolution matrix, fallback window options, window-action verb validation), `tests/window-readiness.test.ts` (epoch binding, stale-report rejection, reload invalidation, fail-safe firing and cancellation), and `tests/electron-api-contract.test.ts` (preload surface includes `windowAction` and epoch-bound `windowReady`).
