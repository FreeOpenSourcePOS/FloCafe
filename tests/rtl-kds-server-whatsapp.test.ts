...
async function run(): Promise<void> {
  ...
  for (const ltrLang of ['en', 'es', 'fr', 'pt', 'it', 'ja', 'zh', 'zh-tw', 'ko', 'id', 'nl', 'hi', 'bn', 'sq'] as const) {
  ...
  const getTestTranslator = (lang: 'en' | 'es' | 'fr' | 'pt' | 'fa' | 'it' | 'ja' | 'zh' | 'zh-tw' | 'ko' | 'id' | 'nl' | 'hi' | 'bn' | 'sq') => {
