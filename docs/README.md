...
| [roles-and-permissions.md](roles-and-permissions.md) | Configurable role templates, per-user overrides, protected owner permissions, context policies, and verification. | CURRENT |
...
| [floadmin-telemetry-locale-tax-handoff.md](floadmin-telemetry-locale-tax-handoff.md) | Handoff for the external cloud/floadmin service: new `currency`, `language`, and `tax_pack` fields on the telemetry and registration payloads, plus guidance for the separate self-registration/signup portal to capture the same dimensions. | CURRENT |
...
| [configurable-roles-permissions-plan.md](configurable-roles-permissions-plan.md) | Implemented architecture and retained rollout/test design record for owner-configurable role templates and per-user overrides. | CURRENT DESIGN RECORD |
