...
Beta channel pre-release (#468 runtime upgrade matrix N+1 target). Same content as the 3.3.1 entry below, published under a prerelease version so stable installs never see it.

## [3.3.1] - 2026-08-24

First published as `3.3.1-beta.1`, the beta-channel N+1 target for the runtime upgrade test matrix (#468). Contains the same changes as 3.3.0 plus the commits on `main` since that release (Linux title-bar fallback controls, native title-bar theme sync).
