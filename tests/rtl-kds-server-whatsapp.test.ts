 ...
 *      localized through i18n keys across all supported languages (en/es/fr/pt/fa/ja).
 *
...
async function run(): Promise<void> {
  ...
  for (const lang of ['en', 'es', 'fr', 'pt', 'fa', 'ja'] as const) {
  ...
  for (const ltrLang of ['en', 'es', 'fr', 'pt', 'ja'] as const) {
  ...
  const getTestTranslator = (lang: 'en' | 'es' | 'fr' | 'pt' | 'fa' | 'ja') => {
