...
If you are correcting typos, refining wording, or improving existing translations (such as Spanish `es.json`, French `fr.json`, Brazilian Portuguese `pt.json`, Filipino `fil.json`, Turkish `tr.json`, Persian `fa.json`, German `de.json`, Italian `it.json`, Japanese `ja.json`, Simplified Chinese `zh.json`, Taiwan Traditional Chinese `zh-tw.json`, Korean `ko.json`, Bahasa Indonesia `id.json`, Dutch `nl.json`, Hindi `hi.json`, Bengali/Bangla `bn.json`, or Albanian `sq.json`):
...
- **UI Language:** Controls the text strings in the interface (English, Spanish, French, Portuguese, Persian, Turkish, Filipino, German, Italian, Japanese, Simplified Chinese, Taiwan Traditional Chinese, Korean, Bahasa Indonesia, Dutch, Hindi, Bengali/Bangla, and Albanian) and date/time presentation language (via `useFormatDate` / `useLocale`), allowing operators in any country to view dates in their preferred language.
