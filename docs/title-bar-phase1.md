...
4. The root renderer chrome reads `titleBarMode` from `getStatus()` before mounting minimal min/max/close buttons. It sends a one-shot readiness signal after that status-driven render, so main shows the hidden-frame window only after the root chrome is committed. The buttons are pinned to the trailing edge, styled with logical CSS properties (RTL-safe), and invoke one narrow IPC verb: `windowAction('minimize' | 'toggle-maximize' | 'close')` exposed via `main/preload.ts`. Context isolation is unchanged.
