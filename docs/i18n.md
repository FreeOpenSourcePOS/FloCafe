...
If you are correcting typos, refining wording, or improving existing translations (such as Spanish `es.json`, French `fr.json`, Brazilian Portuguese `pt.json`, Filipino `fil.json`, Turkish `tr.json`, Persian `fa.json`, German `de.json`, or Japanese `ja.json`):
...
- **UI Language:** Controls the text strings in the interface (English, Spanish, French, Portuguese, Persian, Turkish, Filipino, German, and Japanese) and date/time presentation language (via `useFormatDate` / `useLocale`), allowing operators in any country to view dates in their preferred language.
