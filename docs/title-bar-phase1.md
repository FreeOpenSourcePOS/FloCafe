...
**Phase 2 update (#458):** the overlay colors resolve from shared tokens in `main/title-bar-theme.ts`, and macOS traffic lights sit vertically centered in the 40px bar via `trafficLightPosition`. The runtime theme-sync helper remains available for future dark-theme work, but the current renderer is intentionally light-only and the app pins the native overlay to the light palette (see #513).
