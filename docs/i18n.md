...
If you are correcting typos, refining wording, or improving existing translations (such as Spanish `es.json`, French `fr.json`, Brazilian Portuguese `pt.json`, Russian `ru.json`, Filipino `fil.json`, Turkish `tr.json`, Persian `fa.json`, Arabic `ar.json`, Urdu `ur.json`, German `de.json`, Italian `it.json`, Japanese `ja.json`, Simplified Chinese `zh.json`, Taiwan Traditional Chinese `zh-tw.json`, Korean `ko.json`, Bahasa Indonesia `id.json`, Dutch `nl.json`, Hindi `hi.json`, Bengali/Bangla `bn.json`, Albanian `sq.json`, or Vietnamese `vi.json`):
...
- **UI Language:** Controls the text strings in the interface (English, Spanish, French, Portuguese, Russian, Persian, Arabic, Urdu, Turkish, Filipino, German, Italian, Japanese, Simplified Chinese, Taiwan Traditional Chinese, Korean, Bahasa Indonesia, Dutch, Hindi, Bengali/Bangla, Albanian, and Vietnamese) and date/time presentation language (via `useFormatDate` / `useLocale`), allowing operators in any country to view dates in their preferred language.
...
Translating FloCafe into Russian, Urdu, Albanian, Italian, Japanese, Simplified Chinese, Taiwan Traditional Chinese, Korean, German, Dutch, Hindi, Bengali/Bangla, or Vietnamese does not require creating a tax pack, and adding a tax pack for a new country does not require adding a new UI language.
