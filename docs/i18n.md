...
- **UI Language:** Controls the text strings in the interface (English, Spanish, French, Portuguese, Persian, Turkish, Filipino, German, Italian, Japanese, Simplified Chinese, Korean, and Bahasa Indonesia) and date/time presentation language (via `useFormatDate` / `useLocale`), allowing operators in any country to view dates in their preferred language.
...
Translating FloCafe into Italian, Japanese, Simplified Chinese, Korean, or German does not require creating a tax pack, and adding a tax pack for a new country does not require adding a new UI language.
