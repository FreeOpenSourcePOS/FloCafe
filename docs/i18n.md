...
If you are correcting typos, refining wording, or improving existing translations (such as Spanish `es.json`, French `fr.json`, Brazilian Portuguese `pt.json`, Filipino `fil.json`, Turkish `tr.json`, Persian `fa.json`, Arabic `ar.json`, Urdu `ur.json`, German `de.json`, Italian `it.json`, Japanese `ja.json`, Simplified Chinese `zh.json`, Taiwan Traditional Chinese `zh-tw.json`, Korean `ko.json`, Bahasa Indonesia `id.json`, Dutch `nl.json`, Hindi `hi.json`, Bengali/Bangla `bn.json`, Albanian `sq.json`, or Vietnamese `vi.json`):
...
Translating FloCafe into Urdu, Albanian, Italian, Japanese, Simplified Chinese, Taiwan Traditional Chinese, Korean, German, Dutch, Hindi, Bengali/Bangla, or Vietnamese does not require creating a tax pack, and adding a tax pack for a new country does not require adding a new UI language.
