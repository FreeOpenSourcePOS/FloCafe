...
5. **Decoupled domains:** UI language, tenant regional settings (store timezone and currency), and tax compliance rules are decoupled domains:
   ...
   - **Tenant Settings:** Store-level currency, custom currency symbol and placement, business timezone, calendar system, digit representation, and contact info.
...
- **Country Profiles & Tax Packs:** Located in `main/tax-packs/` and `main/countries.ts`, controlling tax registration number formats, tax rate schedules, default currency symbols, and national fiscal rules. Store timezone remains tenant-authoritative; calendar mode, digit representation, and currency display are constrained by the active country's `localeOptions`.
