...
const languages = ['en', 'es', 'de', 'tr', 'fil', 'fr', 'pt', 'fa', 'it', 'ja', 'zh', 'zh-tw', 'ko', 'id', 'nl', 'hi'] as const;
const order = {
...
async function run(): Promise<void> {
  ...
  for (const language of languages) {
    ...
    assert.match(thermalText, /(?:Time|Hora|Uhrzeit|Saat|Oras|Heure|Ora|時刻|时间|시간|Waktu|Tijd)/, `${language}: thermal time remains visible`);
    assert.match(thermalText, /KITCHEN ORDER TICKET|COMANDA DE COCINA|KUECHENBESTELLSCHEIN|BON DE COMMANDE CUISINE|COMANDA DE COZINHA|BIGLIETTO ORDINE DI CUCINA|キッチン伝票|厨房订单|주방 주문지|TIKET PESANAN DAPUR|KEUKENBESTELBON/, `${language}: thermal banner remains visible`);
