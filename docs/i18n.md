...
If you are correcting typos, refining wording, or improving existing translations (such as Spanish `es.json`, French `fr.json`, Brazilian Portuguese `pt.json`, Filipino `fil.json`, Turkish `tr.json`, Persian `fa.json`, German `de.json`, Japanese `ja.json`, or Simplified Chinese `zh.json`):
...
- **UI Language:** Controls the text strings in the interface (English, Spanish, French, Portuguese, Persian, Turkish, Filipino, German, Japanese, and Simplified Chinese) and date/time presentation language (via `useFormatDate` / `useLocale`), allowing operators in any country to view dates in their preferred language.
