...
- The renderer `TitleBar` component is mounted only by the dashboard layout and only after the existing `window.electronAPI?.getStatus` capability is present. LAN/browser clients therefore receive no Electron-only title-bar markup.
