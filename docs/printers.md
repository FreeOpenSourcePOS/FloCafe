...
On the shared document-driven paths, lines the printer cannot render under the script rules above are skipped with an explicit warning — content is never silently dropped. The printed document itself carries every line plus its text direction, so direction-aware layouts (right-to-left base with left-to-right amounts and order numbers) can be expressed by any renderer without changing the underlying data. The language-policy resolution rules, canonical label catalog flow, bilingual layout strategies, and legacy exceptions are specified in [printing-architecture.md](printing-architecture.md).
